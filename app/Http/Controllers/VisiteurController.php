...
class VisiteurController extends Controller
{
    public function initPasswords(Request $request)
    {
        try {
            ...
            return response()->json(['status_message' => 'mots de passes réinitialiseés']);
        } catch (Exception $e) {
            return response()->json (['sttus_message' => $e->getMessage()], 500);
        }
    }
    ...
    public function login(Request $request)
    {
        if ($request->isJson()) {
            ...
            if (!Auth::attempt($credentials)) {
                return response()->json(['erreur' => 'The provided credentials are incorrect.'], 401);
            }
    ...
    {
        return response()->json(['error' => 'Unathorized acces']);
    }
}
